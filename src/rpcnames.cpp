...
json_spirit::Value
checknamedb (const json_spirit::Array& params, bool fHelp)
{
...
json_spirit::Value
getunotrieinfo (const json_spirit::Array& params, bool fHelp)
{
  if (fHelp || params.size () > 1)
    throw std::runtime_error (
        "getunotrieinfo\n"
  ...
  if (pcoinsTip->HasUnoTrie ())
    ...
    res.push_back (json_spirit::Pair ("error", "not yet implemented"));
